...
int main(int argc, char **argv) {
    ...
    cout << landmarks;
    //just for testing
         Mat tmpimg1=interpolated;
         Mat tmpimg2=image;
         //just enlarge the rowimage
         Mat rawimg=tmpimg1;
         Mat rawimg2;
         int resize_height1 = 480;
         int resize_width1 = 480;
         cv::resize(rawimg,rawimg2,cv::Size(resize_width1,resize_height1),(0,0),(0,0),cv::INTER_NEAREST );
         imwrite("output-raw.png",rawimg2);
          //just landmark
          for(int i=0 ;i < 55; i++)
               cv::circle(tmpimg1, landmarks[i] ,1.5 , cvScalar(255,255,255),-1);
          imwrite("outpath2.png",tmpimg1);
          //just number
          Mat tmpimg3;
          int resize_height = 960;
          int resize_width = 960;
          cv::resize(tmpimg1,tmpimg3,cv::Size(resize_width,resize_height),(0,0),(0,0),cv::INTER_NEAREST );
          imwrite("output-num_chg1.png",tmpimg3);
          string tmpi;
          for(int i=0 ;i < 55; i++)
          {	
               tmpi = to_string(i);
          	   putText(tmpimg3,tmpi,Point(landmarks[i]*10),FONT_HERSHEY_SIMPLEX,1,Scalar(255,255,255),2,2);
          }
          imwrite("output-num_chg2.png",tmpimg3);
 
          for(int i=0 ;i < 55; i++)
               cv::circle(tmpimg2, landmarks[i] ,1.5 , cvScalar(255,255,255),-1);
          imwrite("output3.png",tmpimg2);
    ...
    //just for testing
        ofstream out("landmarks.txt", ofstream::out);
        for(int i = 0; i < 55; i++)
                 out << landmarks[i] << endl;
         out.close();
